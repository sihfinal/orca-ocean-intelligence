#!/usr/bin/env python3
"""
ORCA Pure Technical Architecture & Deployment Blueprint PDF Generator
Generates an exhaustive, developer-focused technical whitepaper containing:
- Complete framework breakdowns (FastAPI, React 18, Vite, TypeScript, Shapely, NumPy, WebSockets, NVIDIA NIM)
- Exact file-by-file directory architecture & code structure
- Complete REST & WebSocket API schemas with JSON request/response payloads
- Mathematical formulations & algorithm pseudo-code
- Step-by-step Local, Render, Vercel, and Edge Docker deployment instructions
- Environment variables, package configurations, and automated test suite specifications.
"""
...
COLOR_ISRO_ORANGE = colors.HexColor("#EA580C")  # ISRO Saffron
...
class TechnicalNumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        # Header
        self.drawString(54, 11 * inch - 36, "ORCA: Technical Architecture, Frameworks & Deployment Specification")
        ...
        self.setFont("Helvetica", 8)
        self.setFillColor(COLOR_MUTED_TEXT)
        self.drawString(225, 32, "• Sih_Hackers • Production Specification")
...
def build_technical_pdf(filename="ORCA_Technical_Architecture_And_Deployment_Report.pdf"):
    # Target printable area: 504 pt width x 684 pt height
    ...
    cover_sub = ParagraphStyle('CSub', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=11, leading=15, textColor=COLOR_SECONDARY, spaceAfter=8)
    ...
    h2_style = ParagraphStyle('H2', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=9.8, leading=13, textColor=COLOR_SECONDARY, spaceBefore=6, spaceAfter=2, keepWithNext=True)
    h3_style = ParagraphStyle('H3', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=8.8, leading=11.5, textColor=COLOR_DARK_TEXT, spaceBefore=4, spaceAfter=2, keepWithNext=True)
    
    body_style = ParagraphStyle('Body', parent=styles['Normal'], fontName='Helvetica', fontSize=8.2, leading=11.6, textColor=COLOR_BODY_TEXT, spaceAfter=3.5)
    bullet_style = ParagraphStyle('Bullet', parent=styles['Normal'], fontName='Helvetica', fontSize=8.0, leading=11.2, textColor=COLOR_BODY_TEXT, leftIndent=9, firstLineIndent=-6, spaceAfter=2)
    callout_style = ParagraphStyle('Callout', parent=styles['Normal'], fontName='Helvetica', fontSize=8.0, leading=11.2, textColor=COLOR_PRIMARY, spaceBefore=1.5, spaceAfter=1.5)
    
    code_inline_style = ParagraphStyle('CodeInline', parent=styles['Normal'], fontName='Courier', fontSize=7.2, leading=9.2, textColor=colors.HexColor("#0F172A"))
    ...
    th_style = ParagraphStyle('TH', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=7.4, leading=9.5, textColor=colors.white, alignment=1)
    td_style = ParagraphStyle('TD', parent=styles['Normal'], fontName='Helvetica', fontSize=7.4, leading=9.5, textColor=COLOR_BODY_TEXT)
    td_bold = ParagraphStyle('TDBold', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=7.4, leading=9.5, textColor=COLOR_BODY_TEXT)
    td_code = ParagraphStyle('TDCode', parent=styles['Normal'], fontName='Courier', fontSize=6.8, leading=8.8, textColor=colors.HexColor("#0F172A"))
    ...
    story.append(Paragraph("ORCA: Technical Stack, Architecture &amp; Deployment Blueprint", cover_title))
    ...
    story.append(HRFlowable(width="100%", thickness=1.2, color=COLOR_ACCENT, spaceBefore=0, spaceAfter=6))
    ...
    # =========================================================================
    # PAGE 2: COMPLETE FILE ARCHITECTURE & RESPONSIBILITIES
    # =========================================================================
...
4. {"type": "PIPELINE_COMPLETE", "payload": { /* Full multi-agent JSON bundle */ }}"""
    ...
    # =========================================================================
    # PAGE 5: COMPLETE DEPLOYMENT GUIDE (LOCAL, RENDER, VERCEL, DOCKER)
    # =========================================================================
    story.append(Paragraph("5. Deployment Engineering &amp; DevOps Guide", h1_style))
    story.append(HRFlowable(width="100%", thickness=1, color=COLOR_PRIMARY, spaceBefore=1, spaceAfter=5))

    story.append(Paragraph(
        "ORCA is engineered for friction-free deployment across <b>Local Development</b>, <b>Production Cloud (Render + Vercel)</b>, and <b>Edge Containerized Nodes</b>:",
        body_style
    ))

    story.append(Paragraph("<b>5.1 Local Development Startup (Unified Launcher)</b>", h2_style))
    story.append(Paragraph("The entire system can be spun up with a single automated master command:", body_style))
    cmd_local = """# 1. Clone repository and install dependencies
git clone https://github.com/Krushna968/RuntimeTerror_SIH_2026.git
cd RuntimeTerror_SIH_2026

# 2. Setup Python environment & install backend requirements
...
# 4. Launch entire stack (FastAPI Backend on :8000 + React Frontend on :5173)
python3 run_system.py"""
    story.append(make_code_box(cmd_local))
    story.append(Spacer(1, 3))

    story.append(Paragraph("<b>5.2 Production Backend Deployment on Render</b>", h2_style))
    story.append(Paragraph("The backend is deployed as a high-concurrency Python Web Service on <b>Render</b>:", body_style))
    render_spec = """• Service Type: Web Service (Native Python Runtime)
• Build Command: pip install -r requirements.txt
• Start Command: python3 -m uvicorn backend.main:app --host 0.0.0.0 --port $PORT --workers 4
• Environment Variables:
    - NVIDIA_API_KEY = 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
    - NVIDIA_MODEL = meta/llama-3.1-8b-instruct
    - PYTHON_VERSION = 3.10.12
• Production URL: https://orca-backend-0dxj.onrender.com
• Health Check Route: GET / (HTTP 200 OK)"""
    story.append(make_code_box(render_spec))
    story.append(Spacer(1, 3))

    story.append(Paragraph("<b>5.3 Production Frontend Deployment on Vercel</b>", h2_style))
    story.append(Paragraph("The client is deployed on <b>Vercel Edge Network</b> with automatic SPA routing rewrites:", body_style))
    vercel_spec = """• Framework Preset: Vite
• Root Directory: client (or workspace root with vercel.json)
• Build Command: npm run build (executing vite build)
• Output Directory: dist
• Environment Variables:
    - VITE_API_URL = https://orca-backend-0dxj.onrender.com
• vercel.json SPA Rewrite Configuration:
    {
      "rewrites": [{ "source": "/(.*)", "destination": "/" }]
    }"""
    story.append(make_code_box(vercel_spec))
    story.append(Spacer(1, 3))

    story.append(Paragraph("<b>5.4 Edge Container Deployment (Docker / Raspberry Pi / NavIC)</b>", h2_style))
    docker_spec = """# Dockerfile for Edge Coastal Station Deployment
FROM python:3.10-slim
WORKDIR /app
RUN apt-get update && apt-get install -y libgeos-dev gcc && rm -rf /var/lib/apt/lists/*
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt
COPY backend/ ./backend/
EXPOSE 8000
CMD ["uvicorn", "backend.main:app", "--host", "0.0.0.0", "--port", "8000"]"""
    story.append(make_code_box(docker_spec))

    story.append(PageBreak())

    # =========================================================================
    # PAGE 6: AUTOMATED VERIFICATION & TECHNICAL BENCHMARKS
    # =========================================================================
    story.append(Paragraph("6. Automated Verification Matrix &amp; Performance Benchmarks", h1_style))
    story.append(HRFlowable(width="100%", thickness=1, color=COLOR_PRIMARY, spaceBefore=1, spaceAfter=5))
    ...
    tech_summary = """<b>Technical Summary:</b> The ORCA codebase delivers a modular, zero-dependency-conflict Agentic AI architecture. By decoupling computationally intensive geospatial vector processing (FastAPI/Shapely/NumPy) from dynamic browser GIS visualization (React/Vite/Leaflet) and offloading cognitive synthesis to NVIDIA NIM Foundation Endpoints with deterministic vernacular fail-safes, ORCA achieves sub-15ms multi-agent reasoning latency with 100% operational uptime."""
    ...
    print(f"✅ Successfully compiled publication-grade technical PDF: {filename}")
